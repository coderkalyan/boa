...
functions: std.SegmentedList(FunctionInfo, 1),
...
pub fn createFunction(pool: *InternPool, fi: FunctionInfo) !FunctionIndex {
    const index: u32 = @intCast(pool.functions.count());
    try pool.functions.append(pool.gpa, fi);
    return @enumFromInt(index);
}

pub fn functionPtr(pool: *InternPool, index: FunctionIndex) *FunctionInfo {
